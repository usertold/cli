      ...
      if (req.method === 'GET' && req.url === '/api/orgs/acme/projects/checkout/coverage-gaps') {
        ...
        res.end(JSON.stringify({
          ...
          gaps: [{
            ...
            target_surface: 'product_under_test',
            signal_type: 'struggling_moment',
            summary: '1 published struggling_moment evidence item on product_under_test has no linked work',
            count: 1,
            evidence_ids: ['sig_1'],
            work_ids: [],
            suggested_action: 'Link the published evidence to existing work or create a reviewed work item.',
          }],
          totals: {
